...
export const signup = async(req,res) => {
    const {email, password, name} = req.body; // for this to work json middleware
    ...
    try {
        ...
        // jwt --> authenticate them in client after we send verification email
...
export const forgotPassword = async(req,res) => {
    ...
    try{
        const user = await User.findOne({email});
        if(!user){
            return res.status(400).json({succress:failure,message : "User Not Found"});
        }
